...
/*
 * 为多个元素分配内存，每个元素都有特定大小
 * 分配的内存必须重置为全零
 */
#define CO_alloc(num, size)             rt_calloc((num), (size))
#define CO_free(ptr)                    rt_free((ptr))
...
#define CO_alloc_break_on_fail(var, num, size) {                    \
    ...
    if((var) != RT_NULL) { mem += (size) * (num); } else { break; } }
...
CO_t *CO_new(CO_config_t *config, uint32_t *heapMemoryUsed) {
    ...
    /* 对于每个对象
     * - 分配内存、验证分配并计算所用堆的大小
     * - 如果定义了 CO_MULTIPLE_OD：
     *   - 使用配置结构
     *   - 计算 CANrx 和 CYNtx 报文的数量：RX_CNT_xx 和 TX_CNT_xx
     *   - 计算索引：RX_IDX_xx 和 TX_IDX_xx
     *   - 计算 CAN 报文缓冲区的总计数： CNT_ALL_RX_MSGS 和 CNT_ALL_TX_MSGS。
     */
